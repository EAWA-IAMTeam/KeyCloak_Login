...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _checkForKeycloakToken() async {
    ...
    logoutBool = html.window.localStorage['logoutBool'] ??
      ...
      Navigator.pushReplacement(
        ...
        MaterialPageRoute(
          builder: (context) => HomePage(
            ...
            googleAccessToken:
                html.window.localStorage['googleAccessToken'] ?? '',
            keycloakRefreshToken:
                html.window.localStorage['keycloakRefreshToken'] ?? '',
            email: keycloakemail,
          ),
        ),
      );
  ...
  Future<Map<String, dynamic>?> _exchangeGoogleTokenForKeycloakTokens(
    ...
    final String serverUrl =
        '${Config.server}:3002/exchange-google-token'; // Go backend endpoint

    final response = await http.post(
      Uri.parse(serverUrl),
      body: json.encode({'googleAccessToken': googleAccessToken}),
    ...
    return Scaffold(
      ...
      body: Center(
        child: ElevatedButton(
          onPressed:
              _checkForKeycloakToken, // Now only triggered when user clicks the button
