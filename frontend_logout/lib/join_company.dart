...
class _JoinCompanyPageState extends State<JoinCompanyPage> {
  ...
  Future<void> _joinGroup(String groupId, String? subgroupId) async {
    ...
    try {
      ...
      print("userid" + userId + "subgroupId: " + subgroupId.toString());

      // Proceed to join the group
      final response = await http.put(
       Uri.parse('${Config.server}:3002/api/adduser?userId=$userId&subgroupId=$targetGroupId'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
  ...
  Future<String?> _getGroupName(String groupId) async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${Config.server}:3002/api/group-details?parentId=$groupId'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
